...
public class Przelewy24TrxResource {
     ...
     * to the query.
     *
     * @param query the query of the przelewy24Trx search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/przelewy-24-trxes")
    @Timed
    public ResponseEntity<List<Przelewy24Trx>> searchPrzelewy24Trxes(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Przelewy24Trxes for query {}", query);
        Page<Przelewy24Trx> page = przelewy24TrxService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/przelewy-24-trxes");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
